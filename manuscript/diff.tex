...
%DIF DEL manuscript-old.tex   Fri Jun  9 20:12:13 2017
%DIF ADD manuscript.tex       Fri Jun  9 20:15:39 2017
%DIF < %% Copernicus Publications Manuscript Preparation Template for LaTeX Submissions
%DIF -------
% Copernicus Publications Manuscript Preparation Template for LaTeX Submissions %DIF > 
%DIF -------
%% ---------------------------------
...
are the only \DIFaddbegin \DIFadd{finite }\DIFaddend bodies for which the self-demagnetization can be treated
...
susceptibility. We also \DIFaddbegin \DIFadd{discuss the ambiguity between confocal ellipsoids
with the same magnetic moment and }\DIFaddend propose a
...
$(x_{c}, y_{c}, z_{c})$, orientation defined by \DIFdelbegin \DIFdel{three angles
}\DIFdelend \DIFaddbegin \DIFadd{the angles
}\textit{\DIFadd{strike}} \DIFaddend $\varepsilon$, \DIFaddbegin \textit{\DIFadd{dip}} \DIFaddend $\zeta$, and \DIFaddbegin \textit{\DIFadd{rake}} \DIFaddend $\eta$
(Fig. \ref{fig:orientation-angles}a), and semi-axes defined by
positive constants $a$, $b$, $c$ (Fig. \ref{fig:orientation-angles}b).
The \DIFdelbegin \DIFdel{angles $\varepsilon$, $\zeta$, and $\eta$ are called }\DIFdelend \DIFaddbegin \DIFadd{orientation angles }\DIFaddend \textit{strike}, \textit{dip}, and \textit{rake}
\DIFdelbegin \DIFdel{, respectively, and }\DIFdelend are commonly used to define the orientation of lines in structural geology
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{clark1986, allmendinger2012}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{pollard2005, allmendinger2012}}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{These unit vectors are commonly described in terms of auxiliary
orientation angles $\alpha$, $\gamma$, and $delta$, which are not used
by the geoscientific community. These angles, however, }\DIFdelend \DIFaddbegin \DIFadd{The matrix $\mathbf{V}$ }\DIFaddend can be defined \DIFdelbegin \DIFdel{from
$\varepsilon$, $\zeta$, and $\eta$ (Fig. \ref{fig:orientation-angles}a)
as follows \mbox{%DIFAUXCMD
\citep{clark1986}}%DIFAUXCMD
:
}\DIFdelend \DIFaddbegin \DIFadd{in terms of three rotation matrices:
}\DIFaddend \begin{equation}
\DIFdelbegin \DIFdel{\alpha }\DIFdelend \DIFaddbegin \DIFadd{\mathbf{R}_{1}(\theta) }\DIFaddend = \DIFdelbegin \DIFdel{\varepsilon - \cos^{-1} }\DIFdelend \left[ \DIFdelbegin \DIFdel{\frac{\cos \eta}{\left( 1 - \sin^{2} \zeta \, \sin^{2} \eta \right)^{\frac{1}{2}}}}\DIFdelend \DIFaddbegin \begin{array}{ccc}
1 & 0 & 0 \\
0 & \cos \theta & \sin \theta \\
0 & -\sin \theta & \cos \theta
\end{array} \DIFaddend \right] \: ,
\DIFdelbegin %DIFDELCMD < \label{eq:alpha}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:R1}
\DIFaddend \end{equation}
\begin{equation}
\DIFdelbegin \DIFdel{\gamma }\DIFdelend \DIFaddbegin \DIFadd{\mathbf{R}_{2}(\theta) }\DIFaddend = \DIFdelbegin \DIFdel{\tan^{-1} }%DIFDELCMD < \left( %%%
\DIFdel{\frac{\cos \zeta}{\sin \zeta \, \cos \eta}}%DIFDELCMD < \right)
%DIFDELCMD < \label{eq:gamma}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \left[ \begin{array}{ccc}
\cos \theta & 0 & -\sin \theta \\
0 & 1 & 0 \\
\sin \theta & 0 & \cos \theta
\end{array} \right]
...
and
\begin{equation}
\DIFdelbegin \DIFdel{\delta }\DIFdelend \DIFaddbegin \DIFadd{\mathbf{R}_{3}(\theta) }\DIFaddend = \DIFdelbegin \DIFdel{\sin^{-1} }%DIFDELCMD < \left( %%%
\DIFdel{\sin \zeta \, \sin \eta }%DIFDELCMD < \right) %%%
\DIFdelend \DIFaddbegin \left[ \begin{array}{ccc}
\cos \theta & \sin \theta & 0 \\
-\sin \theta & \cos \theta & 0 \\
0 & 0 & 1
\end{array} \right] \DIFaddend \: \DIFdelbegin \DIFdel{,
}%DIFDELCMD < \label{eq:delta}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.
}\label{eq:R3}
\DIFaddend \end{equation}
\DIFdelbegin \DIFdel{where $-90^{\circ} \leq \gamma \leq 90^{\circ}$ and
$0 \leq \delta \leq 90^{\circ}$.
Then, given the orientation angles $\varepsilon$, $\zeta$, and $\eta$
(Fig. \ref{fig:orientation-angles}a), we define the auxiliary angles
$\alpha$, $\gamma$ and $\delta$ (Eqs. \ref{eq:alpha}, \ref{eq:gamma}, and
\ref{eq:delta}) and, finally, the unit vectors
$\mathbf{v}_{1}$, $\mathbf{v}_{2}$, and $\mathbf{v}_{3}$
(Fig. \ref{fig:orientation-angles}b) according to the ellipsoid type.
}\DIFdelend For triaxial ellipsoids (i.e., $a > b > c$) and prolate ellipsoids
(i.e., $a > b = c$), we \DIFdelbegin \DIFdel{opted for defining these unit vectors
by following \mbox{%DIFAUXCMD
\citet{clark1986}}%DIFAUXCMD
:
}\begin{displaymath}
\DIFdel{\mathbf{v}_{1} = \left[\begin{array}{c}
                  -\cos\alpha \; \cos\delta \\
                  -\sin\alpha \; \cos\delta \\
                  -\sin\delta
                  \end{array} \right] \: ,
...
%DIFAUXCMD
\begin{displaymath}
\DIFdel{\mathbf{v}_{2} = \left[\begin{array}{c}
...
%DIFAUXCMD
\begin{displaymath}
\DIFdel{\mathbf{v}_{3} = \left[\begin{array}{c}
...
\citet{emerson1985} }%DIFAUXCMD
calculated the unit vectors $\mathbf{v}_{1}$, $\mathbf{v}_{2}$,
and $\mathbf{v}_{3}$ similarly by using Eqs. \ref{eq:v1_triaxial_prolate},
\ref{eq:v2_triaxial_prolate}, and \ref{eq:v3_triaxial_prolate}, but
with $\gamma = 0^{\circ}$.
Finally, we define the unit vectors $\mathbf{v}_{1}$, $\mathbf{v}_{2}$,
and $\mathbf{v}_{3}$ for }\DIFdelend \DIFaddbegin \DIFadd{define the orthogonal matrix $\mathbf{V}$
as follows:
}\begin{equation}
\DIFadd{\mathbf{V} =
...
\DIFadd{For }\DIFaddend oblate ellipsoids (i.e., $a < b = c$)\DIFaddbegin \DIFadd{, we define $\mathbf{V}$ }\DIFaddend as follows:
\begin{equation}
\DIFdelbegin \DIFdel{\mathbf{v}_{1} }\DIFdelend \DIFaddbegin \DIFadd{\mathbf{V} }\DIFaddend =
\DIFdelbegin %DIFDELCMD < \left[\begin{array}{c}
%DIFDELCMD <                   -\cos\alpha \; \sin\gamma \; \sin\delta + \sin\alpha \; \cos\gamma \\
%DIFDELCMD <                   -\sin\alpha \; \sin\gamma \; \sin\delta - \cos\alpha \; \cos\gamma \\
%DIFDELCMD <                   \sin\gamma \; \cos\delta
%DIFDELCMD <                   \end{array} \right] %%%
\DIFdelend \DIFaddbegin \DIFadd{\mathbf{R}_{3} }\left( \DIFadd{-\frac{\pi}{2} }\right) \DIFadd{\;
\mathbf{R}_{1} }\left( \DIFadd{\pi }\right) \DIFadd{\,
\mathbf{R}_{3} }\left( \DIFadd{\varepsilon }\right) \DIFadd{\,
\mathbf{R}_{2} }\left( \DIFadd{\frac{\pi}{2} - \zeta }\right) \DIFadd{\,
\mathbf{R}_{1} }\left( \DIFadd{\eta }\right) \DIFaddend \: \DIFdelbegin \DIFdel{,
}%DIFDELCMD < \label{eq:v1_oblate}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.
}\label{eq:V_oblate}
\DIFaddend \end{equation}
...
%DIFAUXCMD
\DIFdel{This approach is very similar to that presented by \mbox{%DIFAUXCMD
\citet{emerson1985} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{The orthogonal matrices $\mathbf{V}$ used here for triaxial, prolate and oblate
ellipsoids (Eqs. \ref{eq:V_triaxial_prolate} and \ref{eq:V_oblate}) are
different from that used by \mbox{%DIFAUXCMD
\citet{emerson1985} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{clark1986}}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{The unit vectors $\mathbf{u}_{i}$ may be defined
similarly to $\mathbf{v}_{1}$, $\mathbf{v}_{2}$,
and $\mathbf{v}_{3}$ }\DIFdelend \DIFaddbegin \DIFadd{Similarly to the matrix $\mathbf{V}$ }\DIFaddend (Eqs. \DIFdelbegin \DIFdel{\ref{eq:v1_triaxial_prolate},
\ref{eq:v2_triaxial_prolate}, \ref{eq:v3_triaxial_prolate},
\ref{eq:v1_oblate}, \ref{eq:v2_oblate}, and \ref{eq:v3_oblate}), by a given set of }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:ellipsoid_surface},
\ref{eq:V_triaxial_prolate} and \ref{eq:V_oblate}), we define
the matrix $\mathbf{U}$ as a function
of given }\DIFaddend orientation angles $\varepsilon$, $\zeta$,
and $\eta$ \DIFdelbegin \DIFdel{(Eqs. \ref{eq:alpha}, \ref{eq:gamma}, and
\ref{eq:delta}}\DIFdelend \DIFaddbegin \DIFadd{depending on the ellipsoid type. For triaxial
...
(Fig. \ref{fig:orientation-angles}}\DIFaddend ).
...
%DIF < The angular difference $\theta$ between the resultant magnetization
%DIF < and the inducing field is the same, whether the quantities
%DIF < are defined in the main coordinate system (Fig. \ref{fig:orientation-angles}a)
%DIF < or local coordinate system (Fig. \ref{fig:orientation-angles}b).
%DIF < For convenience, we define $\theta$ (in degrees) in the main coordinate system
%DIF < by using the scalar product definition as follows:
%DIF < \begin{equation}
%DIF < \theta = \frac{180}{\pi} \, \cos^{-1} \left(
%DIF < \frac{\mathbf{M}^{\top}\mathbf{H}_{0}}
%DIF < {\| \mathbf{M} \| \; \| \mathbf{H}_{0} \|}\right) \: ,
%DIF < \label{eq:theta}
%DIF < \end{equation}
%DIF < where $\mathbf{M}$ is obtained by pre-multiplying the
%DIF < magnetization $\tilde{\mathbf{M}}$ (Eq. \ref{eq:M-tilde-remanence})
%DIF < by the orthogonal matrix $\mathbf{V}$ (Eq. \ref{eq:A}),
%DIF < which is defined according to the ellipsoid type, and
%DIF < $\mathbf{H}_{0}$ is given by Eq. \ref{eq:H0}.
%DIF < 
...
\DIFaddbegin \subsubsection{\DIFadd{Ambiguity between confocal ellipsoids with the same magnetic moment}}


\DIFadd{There is a fundamental non-uniqueness of ellipsoidal bodies, analogous to the
...
\DIFadd{\tilde{\mathbf{M}} = \chi \left( \mathbf{I} + \chi \, \tilde{\mathbf{N}}^{\dagger} 
\right)^{-1} \tilde{\mathbf{H}}_{0} \: .
\label{eq:M-tilde-K-isotropic}
}\end{equation}
\DIFadd{In this case, the magnetic moment $\tilde{\mathbf{P}}$, defined in the
local coordinate system, is given by
}\begin{equation}
\DIFadd{\tilde{\mathbf{P}} = \vartheta \tilde{\mathbf{M}} \: ,
\label{eq:P-tilde}
}\end{equation}
\DIFadd{where $\vartheta = \frac{4}{3}\pi a b c$ is the ellipsoid volume.
...
\DIFadd{P_{i} = \vartheta \frac{\chi \, H_{0}}{1 + \chi \, \tilde{n}^{\dagger}_{ii}} \: ,
\label{eq:P-tilde-i}
}\end{equation}
\DIFadd{where $H_{0}$ is the intensity of the inducing field $\tilde{\mathbf{H}}_{0}$
...
\DIFadd{Now, consider a confocal ellipsoid with semi-axes $a^{\prime} = \sqrt{a^2+u}$,
...
\DIFadd{\chi^{\prime} = \frac{P_{i}}{\vartheta^{\prime} \, H_{0} - \tilde{n}^{\dagger}_{ii} \, P_{i}} \: ,
\label{eq:chi-prime}
}\end{equation}
\DIFadd{where $\vartheta^{\prime} = \frac{4}{3}\pi a^{\prime} b^{\prime} c^{\prime}$ and
...
\DIFadd{This ambiguity between confocal ellipsoids with the same magnetic moment
...
\DIFaddend \subsection{External magnetic field and total-field anomaly}
...
\DIFaddbegin \DIFadd{All the code developed for generating the results presented in the following
subsections, as well as the code developed for generating additional numerical simulations,
can be found at the folder }\textit{\DIFadd{code}} \DIFadd{of the online repository
}\url{https://github.com/pinga-lab/magnetic-ellipsoid}\DIFadd{.
}


\DIFaddend \subsection{Demagnetizing factors}
...
\subsection{\DIFdelbegin \DIFdel{Simulation of a geological body}\DIFdelend \DIFaddbegin \DIFadd{Confocal ellipsoids}\DIFaddend }


\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{farrar1979} }%DIFAUXCMD
represented the Warrego orebody as a triaxial
ellipsoid having a high isotropic susceptibility.
In this }\DIFdelend \DIFaddbegin \DIFadd{We simulated two confocal ellipsoids by using the parameters
...
are the semi-axes of the Ellipsoid 1 and $u = 2 \times 10^{6}$ }\unit{m}\DIFadd{.
}

\DIFadd{We have computed the total-field anomalies produced by the Ellipsoid 1
and Ellipsoid 2 at the same regular grid of $200 \times 200$ points
located on a horizontal plane at $z = 0$ }\unit{m} \DIFadd{by using two different
inducing fields $\mathbf{H}_{0}$.
}

\DIFadd{In the first case, we used an uniform inducing field $\mathbf{H}_{0}$ which is
parallel to the semi-axis $a$ of the confocal ellipsoids and
has inclination $\approx -4.98^{\circ}$, declination $\approx 15.38^{\circ}$,
and intensity $H_{0} \approx 18.7$ }\unit{A \, m^{-1}}\DIFadd{.
...
In this }\DIFaddend case, the \DIFdelbegin \DIFdel{self-demagnetization strongly impacts the magnetic modelling of this body. According to \mbox{%DIFAUXCMD
\citet{wedekind1990}}%DIFAUXCMD
, the Warrego orebody is a combination of two major and several small ironstone lodes, which are discrete bodies
comprised predominantly of magnetite or hematite above the base of
oxidation.
The Warrego orebody }\DIFdelend \DIFaddbegin \DIFadd{Ellipsoid 2 produces the same total-field
...
\DIFadd{The second inducing field $\mathbf{H}_{0}$ used is oblique to
...
\subsection{\DIFadd{Simulation of a geological body}}


\DIFadd{We simulated an ellipsoidal body similar to the Warrego orebody,
which }\DIFaddend was the resource on which the \DIFaddbegin \DIFadd{well-known }\DIFaddend Warrego mine was
developed \DIFaddbegin \DIFadd{in Tennant Creek, Australia}\DIFaddend . After nearly a decade as
...
\citet{wedekind1990}}%DIFAUXCMD
, the Warrego orebody is a combination of
...
The total-field anomaly varies from \DIFdelbegin \DIFdel{$\approx -176$ }\DIFdelend \DIFaddbegin \DIFadd{$\approx -71$ }\DIFaddend \unit{nT}
to \DIFdelbegin \DIFdel{$\approx 237$ }\DIFdelend \DIFaddbegin \DIFadd{$\approx 482$ }\DIFaddend \unit{nT}, resulting in a peak-to-peak amplitude
of \DIFdelbegin \DIFdel{$\approx 413$ }\DIFdelend \DIFaddbegin \DIFadd{$\approx 553$ }\DIFaddend \unit{nT}, and was calculated by using the
...
}\DIFdelend %DIF > This result reinforces that the use of the approximated magnetization
%DIF > $\breve{\mathbf{M}}$ (Eq. \ref{eq:M-approx}) may negatively impact
%DIF > the magnetic modelling of bodies with high isotropic susceptibility.
...
\DIFdelbegin \DIFdel{$\| \delta \mathbf{M} \| \, \| \mathbf{M} \|^{-1} \approx 2.8 \%$
}\DIFdelend \DIFaddbegin \DIFadd{$\| \delta \mathbf{M} \| \, \| \mathbf{M} \|^{-1} \approx 0.8 \%$
}\DIFaddend (Eq. \ref{eq:perturbed-M-inequality}) in the magnetization.
The peak-to-peak amplitude of the differences in the total-field anomaly
(Fig. \ref{fig:tfa-differences-warrego}c) is \DIFdelbegin \DIFdel{$\approx 1.1$ }\DIFdelend \DIFaddbegin \DIFadd{$\approx 0.3$ }\DIFaddend \unit{nT},
which represents \DIFdelbegin \DIFdel{$\approx 3.3 \%$ }\DIFdelend \DIFaddbegin \DIFadd{$\approx 0.7 \%$ }\DIFaddend of the peak-to-peak amplitude of the
total-field anomaly calculates by using the true magnetization $\mathbf{M}$
(Eqs. \ref{eq:M-K-isotropic} and \ref{eq:M-K-isotropic-inverse-Lambda}).
...
oblate ellipsoids.
We also present a \DIFaddbegin \DIFadd{numerical simulation confirming the
ambiguity between confocal ellipsoids with the same magnetic moment
and present a }\DIFaddend theoretical discussion about the
...
to \DIFdelbegin \DIFdel{to bound }\DIFdelend \DIFaddbegin \DIFadd{determine bounds for }\DIFaddend the maximum relative error in the
magnetic field calculated by neglecting the self-demagnetization.

This work also \DIFdelbegin \DIFdel{provide }\DIFdelend \DIFaddbegin \DIFadd{provides }\DIFaddend a set of routines to model the magnetic
field produced by ellipsoids.
The routines are written in \DIFaddbegin \DIFadd{the }\DIFaddend Python language as part of
the Fatiando a Terra \citep{uieda2013}
\DIFdelbegin \DIFdel{, which is an
}\DIFdelend open-source library for modelling and inversion in geophysics.
The current version of our code is freely distributed
in a repository \DIFdelbegin \DIFdel{of GitHub }\DIFdelend \DIFaddbegin \DIFadd{hosted on the GitHub website}\DIFaddend . We are working to integrate our
routines in the \DIFdelbegin \DIFdel{following }\DIFdelend \DIFaddbegin \DIFadd{next }\DIFaddend stable release of Fatiando a Terra.
We hope that these routines \DIFaddbegin \DIFadd{will }\DIFaddend be useful to the wide geoscientific
community\DIFdelbegin \DIFdel{either in researching and }\DIFdelend \DIFaddbegin \DIFadd{, either for research or for }\DIFaddend teaching.
...
between \DIFdelbeginFL \DIFdelFL{"minus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL $x$\DIFdelbeginFL \DIFdelFL{" }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-axis }\DIFaddendFL and the strike direction is called \textit{strike}.
The angle $\zeta$ between the horizontal plane and the dark gray plane
is called \textit{dip}. The \DIFdelbeginFL \DIFdelFL{line containing the unit vector
$\mathbf{u}$ defines the }\textit{\DIFdelFL{plunge direction}}%DIFAUXCMD
\DIFdelFL{.
The }\DIFdelendFL angle $\eta$ between the strike direction
and the \DIFdelbeginFL \DIFdelFL{plunge direction
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{line containing the unit vector $\mathbf{u}$
}\DIFaddendFL is called \textit{rake}. The projection of \DIFdelbeginFL \DIFdelFL{the plunge direction }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{this line }\DIFaddendFL on the
horizontal plane \DIFaddbeginFL \DIFaddFL{(not shown) }\DIFaddendFL is called \DIFdelbeginFL \textit{\DIFdelFL{plunge azimuth direction}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{dip direction}}
\DIFaddFL{\mbox{%DIFAUXCMD
\citep{pollard2005, allmendinger2012}}%DIFAUXCMD
}\DIFaddendFL . (b) Local coordinate system
...
%f
\begin{figure}[t]
\DIFaddbeginFL \includegraphics[width=8.3cm]{figures/confocal_ellipsoids_parallel_field.pdf}
\caption{\DIFaddFL{Total-field anomaly (in }\unit{nT}\DIFaddFL{) produced by the 
...
at the constant vertical coordinate $z = 0$ }\unit{m}\DIFaddFL{.
...
\caption{\DIFaddFL{Total-field anomalies (in }\unit{nT}\DIFaddFL{) produced by the (a) Ellipsoid 1
...
%DIF > t
\begin{table}[t]
\caption{\DIFaddFL{Parameters defining two confocal ellipsoids.}}
%DIF > \begin{tabular}{column = lcr}
\begin{tabular}{lccr}
\tophline
\DIFaddFL{Parameter }& \DIFaddFL{Ellipsoid 1 }& \DIFaddFL{Ellipsoid 2 }& \DIFaddFL{Unit }\\
\middlehline
\DIFaddFL{Semi-axis $a$ }& \DIFaddFL{$900$ }& \DIFaddFL{$\approx 1676.31$ }& \unit{m} \\
\DIFaddFL{Semi-axis $b$ }& \DIFaddFL{$500$ }& \DIFaddFL{$1500$ }& \unit{m} \\
\DIFaddFL{Semi-axis $c$ }& \DIFaddFL{$100$ }& \DIFaddFL{$\approx 1417.74$ }& \unit{m} \\
\DIFaddFL{Coordinate of the centre $x_{c}$ }& \DIFaddFL{0 }& \DIFaddFL{0 }& \unit{m} \\
\DIFaddFL{Coordinate of the centre $y_{c}$ }& \DIFaddFL{0 }& \DIFaddFL{0 }& \unit{m} \\
\DIFaddFL{Coordinate of the centre $z_{c}$ }& \DIFaddFL{1500 }& \DIFaddFL{1500 }& \unit{m} \\
\DIFaddFL{Orientation angle $\varepsilon$ $^{\ast}$ }& \DIFaddFL{$45$ }& \DIFaddFL{$45$ }& \DIFaddFL{$^{\circ}$ }\\
\DIFaddFL{Orientation angle $\zeta$ $^{\ast}$ }& \DIFaddFL{$10$ }& \DIFaddFL{$10$ }& \DIFaddFL{$^{\circ}$ }\\
\DIFaddFL{Orientation angle $\eta$ $^{\ast}$ }& \DIFaddFL{$-30$ }& \DIFaddFL{$-30$ }& \DIFaddFL{$^{\circ}$ }\\
\DIFaddFL{Isotropic susceptibility $\chi$ }& \DIFaddFL{$1.2$ }& \DIFaddFL{$\approx 0.014$ }& \unit{SI} \\
\bottomhline
\end{tabular}
\belowtable{$^{\ast}$ Defined in Fig. \ref{fig:orientation-angles}a} %DIF >  Table Footnotes
