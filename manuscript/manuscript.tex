...
ellipsoids which have both induced and remanent magnetization, as 
well as isotropic or anisotropic susceptibility. We also propose a 
...
of routines a set of routines to model the magnetic field produced 
...
$(x_{c}, y_{c}, z_{c})$, orientation defined by three angles 
$\varepsilon$, $\zeta$, and $\eta$ (Fig. \ref{fig:orientation-angles}a), 
and semi-axes defined by
positive constants $a$, $b$, $c$ (Fig. \ref{fig:orientation-angles}b).
The angles $\varepsilon$, $\zeta$, and $\eta$ are called \textit{strike}, 
\textit{dip}, and \textit{rake}, respectively, and are commonly used to
define the orientation of lines in structural geology
\citep{clark1986, allmendinger2012}.
...
These unit vectors are commonly described in terms of auxiliary 
orientation angles $\alpha$, $\gamma$, and $delta$, which are not used
by the geoscientific community. These angles, however, can be defined from
$\varepsilon$, $\zeta$, and $\eta$ (Fig. \ref{fig:orientation-angles}a)
...
\label{eq:delta}
\end{equation}
where $-90^{\circ} \leq \gamma \leq 90^{\circ}$ and
$0 \leq \delta \leq 90^{\circ}$.
Then, given the orientation angles $\varepsilon$, $\zeta$, and $\eta$ (Fig. \ref{fig:orientation-angles}a and Eqs. \ref{eq:alpha}, \ref{eq:gamma}, and
\ref{eq:delta}), we define the unit vectors
$\mathbf{v}_{1}$, $\mathbf{v}_{2}$, and $\mathbf{v}_{3}$
(Fig. \ref{fig:orientation-angles}b) according to the ellipsoid type.
For triaxial ellipsoids (i.e., $a > b > c$), these unit vectors are 
given by \citep{clark1986}:
\begin{equation}
...
\end{equation}
Similarly, the unit vectors $\mathbf{v}_{1}$, $\mathbf{v}_{2}$, and $\mathbf{v}_{3}$
for prolate ellipsoids (i.e., $a > b = c$) are calculated
by using Eqs. \ref{eq:v1_triaxial_prolate}, \ref{eq:v2_triaxial_prolate}, and
\ref{eq:v3_triaxial_prolate}, but with $\gamma = 0^{\circ}$
\citep{emerson1985}.
Finally, the unit vectors $\mathbf{v}_{1}$, $\mathbf{v}_{2}$, and $\mathbf{v}_{3}$
for oblate ellipsoids (i.e., $a < b = c$) are calculated as follows:
\begin{equation}
\mathbf{v}_{1} = \left[\begin{array}{c} 
...
\label{eq:v3_oblate}
\end{equation}
These formulas are similar to those presented by \citep{emerson1985}.
...
strength $\leq 1$ \unit{mT} \citep{rochette1992}.
...
The unit vectors $\mathbf{u}_{i}$ may be defined
similarly to $\mathbf{v}_{1}$, $\mathbf{v}_{2}$,
and $\mathbf{v}_{3}$ (Eqs. \ref{eq:v1_triaxial_prolate},
\ref{eq:v2_triaxial_prolate}, \ref{eq:v3_triaxial_prolate},
\ref{eq:v1_oblate}, \ref{eq:v2_oblate}, and \ref{eq:v3_oblate}),
by a given set of orientation angles $\varepsilon$, $\zeta$,
and $\eta$ (Eqs. \ref{eq:alpha}, \ref{eq:gamma}, and
\ref{eq:delta}).
...
%The angular difference $\theta$ between the resultant magnetization
%and the inducing field is the same, whether the quantities
%are defined in the main coordinate system (Fig. \ref{fig:orientation-angles}a)
%or local coordinate system (Fig. \ref{fig:orientation-angles}b).
%For convenience, we define $\theta$ (in degrees) in the main coordinate system
%by using the scalar product definition as follows:
%\begin{equation}
%\theta = \frac{180}{\pi} \, \cos^{-1} \left( 
%\frac{\mathbf{M}^{\top}\mathbf{H}_{0}}
%{\| \mathbf{M} \| \; \| \mathbf{H}_{0} \|}\right) \: ,
%\label{eq:theta}
%\end{equation}
%where $\mathbf{M}$ is obtained by pre-multiplying the
%magnetization $\tilde{\mathbf{M}}$ (Eq. \ref{eq:M-tilde-remanence})
%by the orthogonal matrix $\mathbf{V}$ (Eq. \ref{eq:A}),
%which is defined according to the ellipsoid type, and
%$\mathbf{H}_{0}$ is given by Eq. \ref{eq:H0}.
%
...
For readers interested in automated code testing,
we recommend the freely reusable lessons of the Software Carpentry 
(\url{https://software-carpentry.org/}) and at the
description of the doctest module available
in the documentation for Python 2.X 
(\url{https://docs.python.org/2/library/doctest.html}).

The numerical simulations presented here was generated with
the Jupyter Notebook
(\url{http://jupyter-notebook.readthedocs.io/en/latest/notebook.html}),
which is a web application that allows creating and sharing
documents that contain live code, equations, visualizations and
explanatory text.
Besides using the Fatiando a Terra \citep{uieda2013},
the numerical simulations use the 
NumPy library \citep{vanderwalt2011} to perform numerical
computations, the Matplotlib library \citep{hunter2007} to plot
the results and generate figures.
The Jupyter Notebooks used to reproduce all the results presented here
are available at a repository in the GitHub
...
oxidation.
The Warrego orebody was the resource on which the Warrego mine was
developed. After nearly a decade as one of the most important 
gold and copper mines in Australia, the Warrego mine was closed 
in late 1989.
...
The total-field anomaly varies from $\approx -295.3$ \unit{nT}
to $\approx 149.5$ \unit{nT}, resulting in a peak-to-peak amplitude
of $\approx 444.9$ \unit{nT}, and was calculated by using the 
...
The peak-to-peak amplitude is $\approx 212.0$ \unit{nT}
and represents $\approx 48 \%$ of the peak-to-peak amplitude of
...
$\| \delta \mathbf{M} \| \, \| \mathbf{M} \|^{-1} \approx 4.2 \%$
...
(Fig. \ref{fig:tfa-differences-warrego}b) is $\approx 1.5$ \unit{nT},
which represents $\approx 4.5 \%$ of the peak-to-peak amplitude of the
...
$\| \delta \mathbf{M} \| \, \| \mathbf{M} \|^{-1} \approx 4.9 \%$
(Eq. \ref{eq:perturbed-M-inequality}) in the magnetization.
The peak-to-peak amplitude of the differences in the total-field anomaly
(Fig. \ref{fig:tfa-differences-warrego}c) is $\approx 2.0$ \unit{nT},
which represents $\approx 5.1 \%$ of the peak-to-peak amplitude of the
total-field anomaly calculates by using the true magnetization $\mathbf{M}$
(Eqs. \ref{eq:M-K-isotropic} and \ref{eq:M-K-isotropic-inverse-Lambda}).
...
by the results obtained with numerical simulations.
In a future work,
it would be interesting to use a similar approach to
to bound the maximum relative error in the
magnetic field calculated by neglecting the self-demagnetization.

This work also provide a set of routines to model the magnetic 
field produced by ellipsoids.
The routines are written in Python language as part of
the Fatiando a Terra \citep{uieda2013}, which is an 
open-source library for modelling and inversion in geophysics.
The current version of our code is freely distributed
in a repository of GitHub. We are working to integrate our
routines in the following stable release of Fatiando a Terra.
We hope that these routines be useful to the wide geoscientific
community either in researching and teaching.


\section{Code availability}


The current version of our code is freely distributed, under 
BSD 3-clause licence, in a repository of GitHub
(\url{https://github.com/pinga-lab/magnetic-ellipsoid}).
The code was developed as part of the Fatiando a Terra 
\citep{uieda2013}, which is an open-source Python 
library for modelling and inversion in geophysics.
Instructions for running the current version of our code
in the Fatiando a Terra are provided at its repository
in GitHub.
Documentation and installation instructions for 
the most current release version of Fatiando a Terra 
are provided at \url http://www.fatiando.org.
...
between "minus $x$" and the strike direction is called \textit{strike}.
The angle $\zeta$ between the horizontal plane and the dark gray plane
is called \textit{dip}. The line containing the unit vector
$\mathbf{u}$ defines the \textit{plunge direction}. 
The angle $\eta$ between the strike direction and the plunge direction
is called \textit{rake}. The projection
of the plunge direction on the horizontal plane is called
\textit{plunge azimuth direction}. (b) Local coordinate system
