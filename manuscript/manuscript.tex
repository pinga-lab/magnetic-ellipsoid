...
\runningauthor{D. Takahashi and V. C. Oliveira Jr.}
...
axes of a body.
This a function of the body shape and gives rise
...
Australia.
Later, \citet{hoschke1991} also showed how the
...
of self-demagnetisation led to errors of $\approx 55^{\circ}$
...
different from those used by \citet{emerson1985} and \citet{clark1986}.
...
system as the \textit{local coordinate system}.
...
(Fig. \ref{fig:orientation-angles}b) are indicated with
the symbol "$\sim$".
...
may be different from the angles $\varepsilon$, $\zeta$
...
\label{eq:nij}
\end{equation}
where $r_{1} = x$, $r_{2} = y$, $r_{3} = z$ are the elements of
...
is called the \textit{depolarization tensor} \citep{soliverez1981, soliverez2008, soliverez2016}.
...
Note that, according to Eqs. \ref{eq:H-tilde}
...
For triaxial ellipsoids (i.g. $a > b > c$), the demagnetizing
...
second kind, respectively. \citet{stoner1945} presented a
...
For prolate ellipsoids (i.e. $a > b = c$), the demagnetizing
factors obtained by solving Eq. \ref{eq:n-tilde-dagger-ii}
are given by
\begin{equation}
...
These formulas were later presented by \citet{emerson1985}.
...
For oblate ellipsoids (i.e. $a < b = c$), the demagnetizing
factors obtained by solving Eq. \ref{eq:n-tilde-dagger-ii}
are given by
\begin{equation}
\tilde{n}^{\dagger}_{11} =
...
\label{eq:gi}
\end{equation}
where $e_{1} = a$, $e_{2} = b$, $e_{3} = c$ and
...
For triaxial ellipsoids (i.e. $a > b > c$), the functions
...
For prolate (i.e. $a > b = c$) ellipsoids, the functions
...
For oblate (i.e. $a < b = c$) ellipsoids, the functions
$g_{i}$ (Eq. \ref{eq:gi}) are given by
\begin{equation}
...
volume $\vartheta$ of the ellipsoid and using the Maxwell postulate
...
$\tilde{\mathbf{N}}^{\dagger}$ is as defined in the
previous section.
...
many authors
\citep{maxwell1873, dubois1896, stoner1945, clark1986, sttraton2007},
...
as part of the open-source library
...
which is a web application that allows the creation and sharing of
...
Also, Fig. \ref{fig:demag-factors}a confirms that the demagnetizing
...
The semi-axes of Ellipsoid 2 were defined as $\sqrt{a^{2} + u}$,
$\sqrt{b^{2} + u}$ and $\sqrt{c^{2} + u}$, where $a$, $b$, and $c$
are the semi-axes of Ellipsoid 1 and $u = 2 \times 10^{6}$ \unit{m}.
...
In the first case, we used a uniform inducing field $\mathbf{H}_{0}$ which is
...
The differences are shown in Fig. \ref{fig:tfa-confocal-ellipsoids-H0-oblique}c.
...
which was the resource on which the well-known Warrego mine 
...
total-field anomaly calculated by using the true magnetization $\mathbf{M}$
(Eqs. \ref{eq:M-K-isotropic} and \ref{eq:M-K-isotropic-inverse-Lambda}).
...
Zenodo:  \url{http://doi.org/10.5281/zenodo.825261} \citep{takahashi2017}. The latest
...
to the ellipsoid type. Notice that
...
Also, the depolarization tensor $\mathbf{N}(\mathbf{r})$
...
with that defined in \ref{eq:reference-triaxial-ell} and pass through
...
$u$ for Eq. \ref{eq:quadric-confocal-prolate-ell}
to represent an ellipsoid is
\begin{equation}
u + b^{2} > 0 \: .
...
D. Takahashi thanks the brazilian research founding agency
...
%f
\begin{figure}[t]
\includegraphics[width=8.3cm]{figures/f01.png}
...
The dark grey plane contains the centre $(x_{c}, y_{c}, z_{c})$
...
which contains the $x$ axis and $y$ axis. The angle $\varepsilon$
between the $x$ axis and the strike direction is called \textit{strike}.
The angle $\zeta$ between the horizontal plane and the dark grey plane
...
\caption{Total-field anomalies (in \unit{nT}) produced by (a) Ellipsoid 1
...
%f
\begin{figure}[t]
\includegraphics[width=8.3cm]{figures/f05.pdf}
...
%f
\begin{figure}[t]
\includegraphics[width=8.3cm]{figures/f06.pdf}
...
based on that presented by \citet{farrar1979} to simulate
